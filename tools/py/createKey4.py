...
def createKey(data,comKeyList): 
    keyArr=[]
    for item in data:
        enVal=item[2]

        #用简写单词替换
        print('old enVal:',enVal)
        keyArr=enVal.split(' ')
        newKeyArr=[]
        for keyItem in keyArr:
            for comKeyItem in comKeyList:
                regex=re.compile(comKeyItem[1],re.IGNORECASE) #re.IGNORECASE 查找时忽视大小写
                # print('old keyItem:',keyItem) 
                keyItem=re.sub(regex,comKeyItem[0],keyItem)  
                # print('new keyItem:',keyItem)
            
            newKeyArr.append(keyItem) 
        enVal=' '.join(newKeyArr)
        print('new enVal:',enVal)      
...
if __name__ =="__main__": 
    langData=createKey(data,commonKey)
