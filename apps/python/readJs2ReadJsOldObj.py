...
# ReadOldObj:自动添加 import { mapState } from 'vuex'
def autoStyleIsFlag():
    dirList=getReplaceDirList()
    for item in dirList:
        if item[1]:
            file=item[2]
            with io.open(file, "r", encoding="utf-8") as f:
                str=f.read()
                importStr="import { mapState } from 'vuex'"
                if(str.find('<script>')!=-1):
                    str=str.replace('<script>'+'\n','<script>'+'\n'+importStr+'\n')
            with io.open(file,"w",encoding="utf-8") as f:
                f.write(str)
...
def replaceText():
    ...
    autoStyleIsFlag()#自动添加 import { mapState } from 'vuex'
